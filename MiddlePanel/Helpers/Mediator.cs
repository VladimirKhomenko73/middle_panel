...
namespace MiddlePanel.Helpers
{
    ...
    public class Mediator
    {
        ...
        /// <summary>
        /// This is aproximation method.
        /// </summary>
        /// <param name="prevValue">Previous value</param>
        /// <param name="newValue">New value</param>
        /// <param name="valueName">Value key</param>
        private void Aproximation(object prevValue, object newValue, string valueName)
        {
            int iterations = Convert.ToInt32(60/30);
            int iterationNumber = 0;
            double iterationStep = ((double)newValue - (double)prevValue) / iterations;
            double currentValue = (double)prevValue;
            while (iterationNumber < iterations)
            {
                currentValue = currentValue + iterationStep;
                NotifyColleagues((object)valueName, new Parameter(valueName,currentValue));
        ...
        /// <summary>
        /// This method broadcasts a message to all message targets for a given
        /// message key and passes a parameter. Aproximation included.
        /// </summary>
        /// <param name="key">Message key</param>
        /// <param name="message">Message parameter</param>
        /// <returns>True/False if any handlers were invoked.</returns>
        public bool NotifyColleagues<T>(string key, T message)
        {
            ...
            Parameter param = (Parameter)messageParam;
            if (param.GetValue() is double)
            {
                if (parameterBuffer.ContainsKey(key))
                {
                    Task task = new Task(()=>Aproximation(parameterBuffer[key].GetValue(), param.GetValue(), key));
                    task.Start();
                    return true;
                }
                else
